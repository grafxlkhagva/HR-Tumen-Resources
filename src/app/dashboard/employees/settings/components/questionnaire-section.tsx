...
function CategoryCard({ category, companyData, companyLoading }: { category: CategoryConfig; companyData: SimpleReferenceItem[] | null; companyLoading: boolean }) {
    const Icon = category.icon;
    ...
    // Fetch global reference data for this category
    const { data: globalData, isLoading: globalLoading } = useGlobalReferenceData(category.collectionName);

    const isLoading = companyLoading || globalLoading;
    const globalCount = globalData?.length || 0;
    const companyCount = companyData?.length || 0;
    const totalCount = globalCount + companyCount;

    // Merged items for display: global (read-only) + company (editable)
    const mergedData = React.useMemo(() => {
        ...
        // Filter out company items that duplicate global items by name
        const globalNames = new Set(globalItems.map(i => (i.name || '').toLowerCase()));
        const uniqueCompanyItems = companyItems.filter(
            item => !globalNames.has((item.name || '').toLowerCase())
        );
        return [...globalItems, ...uniqueCompanyItems];
    }, [globalData, companyData]);

    const existingItems = React.useMemo(() => {
        return mergedData.map(item => item.name || '').filter(Boolean);
    }, [mergedData]);
            ...
            <Card className={cn("border shadow-sm hover:shadow-md transition-all", category.borderColor)}>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <TooltipProvider>
                                <Tooltip>
                                    ...
                                    <TooltipContent>
                                        <p>Глобал: {globalCount} | Байгууллагын: {companyCount}</p>
                                    </TooltipContent>
                                </Tooltip>
                            </TooltipProvider>
                        </div>
                ...
                <CardContent className="pt-0">
                    <ReferenceTable
                        ...
                        itemData={mergedData as SimpleReferenceItem[]}
                        isLoading={isLoading}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {filteredCategories.map((category) => (
                                <CategoryCard
                                    key={category.id}
                                    category={category}
                                    companyData={dataMap[category.id]}
                                    companyLoading={loadingMap[category.id]}
                                />
                            ))}
                        </div>
                    )}
