        ...
        (error) => {
          const description =
            error.code === error.PERMISSION_DENIED
              ? 'Байршлын зөвшөөрөл хориглогдсон байна.'
              : error.code === error.POSITION_UNAVAILABLE
              ? 'Байршлын мэдээлэл авах боломжгүй байна.'
              : error.code === error.TIMEOUT
              ? 'Байршил авах хугацаа хэтэрсэн байна.'
              : 'Тодорхойгүй алдаа гарлаа.';
          toast({ variant: 'destructive', title: 'Байршил авахад алдаа гарлаа', description });
        }
      );
    } else {
  ...
  const handleSearchAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        handleLocationChange(parseFloat(data[0].lat), parseFloat(data[0].lon));
        toast({ title: 'Байршил олдлоо', description: data[0].display_name });
      } else {
        toast({
          variant: 'destructive',
          title: 'Илэрц олдсонгүй',
          description: 'Та хаягаа шалгаад дахин оролдоно уу.',
        });
      }
    } catch (error) {
