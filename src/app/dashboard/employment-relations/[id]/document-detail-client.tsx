...
import { doc, Timestamp, updateDoc, collection, query, where, getDoc, deleteDoc, orderBy, onSnapshot } from 'firebase/firestore';
...
import { formatDateTime, getReplacementMap, generateDocumentContent, generateDocumentHeader } from '../utils';
import type { ERDocumentType } from '../types';
import { format } from 'date-fns';
import { RichTextEditor } from '../components/rich-text-editor';
import { DynamicFieldSelector } from '../components/dynamic-field-selector';
...
export function DocumentDetailClient({ documentId }: { documentId: string }) {
    ...
    // Fetch template to check permissions and header settings
    ...
    // Fetch document type for header config
    const docTypeRef = useMemo(() => firestore && document?.documentTypeId ? doc(firestore, 'er_process_document_types', document.documentTypeId) : null, [firestore, document?.documentTypeId]);
    const { data: docType } = useDoc<ERDocumentType>(docTypeRef as any);
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div className="xl:col-span-2 space-y-4">
                        ...
                        {/* Document Editor / Preview Card */}
                        <div className="bg-white rounded-xl border overflow-hidden">
                            <div className="px-4 py-3 border-b flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <FileText className="h-4 w-4 text-slate-400" />
                                    <span className="text-sm font-medium">Баримтын агуулга</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    {(currentStatus === 'DRAFT' || currentStatus === 'IN_REVIEW') && docViewMode === 'edit' && (
                                        <Button variant="ghost" size="sm" className="h-7 text-xs text-slate-500" onClick={restoreTemplateContent}>
                                    ...
                                    <TabsContent value="edit" className="mt-0">
                                        <div className="p-4">
                                            <RichTextEditor
                                                ...
                                                placeholder="Баримтын агуулгыг энд засварлана уу..."
                                            />
                                            <div className="mt-3 border rounded-lg p-3 bg-slate-50">
                                                <p className="text-xs font-medium text-slate-500 mb-2">Хувьсагч нэмэх</p>
                                                <DynamicFieldSelector
                                                    onSelect={(field) => {
                                                        setPendingInsertContent(field);
                                                    }}
                                                    customFields={[...(template?.customInputs || [])]
                                                        .sort((a: any, b: any) => (a.order || 0) - (b.order || 0))
                                                        .map((i: any, idx: number) => ({
                                                            key: i.key ? `{{${i.key}}}` : `{{field_${idx}}}`,
                                                            label: i.label || `Field ${idx + 1}`,
