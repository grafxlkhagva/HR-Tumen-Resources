...
import { Users, MoreHorizontal, Pencil, Trash2, ChevronDown, ChevronUp, User, CheckCircle } from 'lucide-react';
import { Department } from '../types';
import { cn } from '@/lib/utils';
import { useState } from 'react';
import { Badge } from '@/components/ui/badge';

export const OrgChartNode = ({ node, isFirst, isLast, isSole, onDepartmentClick, parentLevel = 0 }: { node: Department, isFirst?: boolean, isLast?: boolean, isSole?: boolean, onDepartmentClick?: (deptId: string) => void, parentLevel?: number }) => {
    const [isExpanded, setIsExpanded] = useState(true);
    const hasChildren = node.children && node.children.length > 0;

    // Calculate vertical offset for level-based positioning
    const currentLevel = node.typeLevel ?? (parentLevel + 1);
    const levelDiff = Math.max(0, currentLevel - (parentLevel + 1));
    const verticalOffset = levelDiff * 80; // 80px per level

    return (
        <li className="relative flex flex-col items-center px-4">
            {/* Upper Connector: Vertical Line Up */}
            <div
                className="absolute -top-4 left-1/2 border-l-2 border-dashed border-muted-foreground/20 -translate-x-1/2"
                style={{ height: `calc(1rem + ${verticalOffset}px)` }}
            ></div>
            ...
            {!isSole && (
                <>
                    ...
                    {!isFirst && (
                        <div className="absolute -top-4 left-0 right-1/2 border-t-2 border-dashed border-muted-foreground/20"></div>
                    )}
                    {/* Right half of the bridge (if not last) */}
                    {!isLast && (
                        <div className="absolute -top-4 left-1/2 right-0 border-t-2 border-dashed border-muted-foreground/20"></div>
                    )}
                </>
            )}
            ...
            <div
                ...
                className={cn(
                    "relative z-10 w-60 rounded-xl border border-border/50 bg-card p-5 text-center text-card-foreground shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 group cursor-pointer hover:border-border",
            ...
            >
                <div className="space-y-1.5">
                    <div className="flex items-center justify-center gap-2 min-h-[40px]">
                        <p className="font-semibold text-sm tracking-tight line-clamp-2">
                            {node.name}
                        </p>
                        {node.status === 'active' && <CheckCircle className="h-3.5 w-3.5 text-emerald-500 shrink-0" />}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground bg-muted/20 px-2 py-0.5 rounded-full border border-border/50">
                            ...
                            <span className="font-semibold">{node.approvedCount || 0}</span>
                        </div>
                    </div>
                </div>
            ...
            {hasChildren && isExpanded && (
                <>
                    ...
                    <div className="absolute top-full h-4 border-l-2 border-dashed border-muted-foreground/20"></div>

                    {/* Children Container */}
                    <ul className="relative mt-4 flex justify-center gap-8 pt-4">
    ...
    return (
        <li className="relative flex flex-col items-center">
            <div
                onClick={() => onDepartmentClick?.(node.id)}
                className={cn(
                    "relative z-10 w-64 rounded-xl border border-border/50 bg-card p-6 text-center text-card-foreground shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 group cursor-pointer hover:border-border",
                    !isExpanded && hasChildren && "border-b-4"
                )}
                style={{
            ...
            >
                <div className="space-y-2">
                    <div className="flex items-center justify-center gap-2">
                        ...
                        {node.status === 'active' && <CheckCircle className="h-4 w-4 text-emerald-500 shrink-0" />}
                    </div>
