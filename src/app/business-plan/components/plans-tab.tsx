// src/app/dashboard/hr/business-plan/components/plans-tab.tsx
'use client';

import React, { useState, useMemo } from 'react';
import Link from 'next/link';
import { useFirebase, addDocumentNonBlocking, updateDocumentNonBlocking, useTenantWrite, useUser } from '@/firebase';
...
export function PlansTab({ plans, themes, employees, companyProfile, coreValues, isLoading }: PlansTabProps) {
    ...
    const handleCreatePlan = async (values: BusinessPlanFormValues) => {
        if (!firestore || !user) return;
        try {
            const docRef = await addDoc(tCollection('bp_plans'), {
                ...values,
                createdAt: new Date().toISOString(),
                createdBy: user.uid,
            });

            // BSC-д 4 default perspective автоматаар үүсгэнэ
            if (values.framework === 'bsc' && docRef) {
                await Promise.all(
                    BSC_DEFAULT_PERSPECTIVES.map((persp, idx) =>
                        addDoc(tCollection('bp_themes'), {
                            planId: docRef.id,
                            title: persp.title,
            ...
            toast({
                variant: 'destructive',
                title: 'Алдаа',
                description: err?.message || 'Төлөвлөгөө үүсгэхэд алдаа гарлаа.',
            });
        }
    };
    ...
    // Шинэ theme нэмэхэд нийлбэр жин 100-аас хэтрэхгүй эсэхийг шалгана
    const validateNewWeight = (newWeight: number, excludeId?: string): boolean => {
        const otherWeights = planThemes
            .filter(t => t.id !== excludeId)
            .reduce((s, t) => s + t.weight, 0);
        return otherWeights + newWeight <= 100;
    };

    const handleCreateTheme = (values: StrategicThemeFormValues) => {
        if (!firestore || !currentPlan) return;
        if (!validateNewWeight(values.weight)) {
            toast({
                variant: 'destructive',
                title: 'Жингийн нийлбэр 100%-иас хэтэрлээ',
                description: `Одоогийн нийт жин ${planThemes.reduce((s, t) => s + t.weight, 0)}%, та ${100 - planThemes.reduce((s, t) => s + t.weight, 0)}%-аас ихгүй жин оруулна уу.`,
            });
            return;
        }
    ...
    const handleUpdateTheme = (values: StrategicThemeFormValues) => {
        ...
        if (!validateNewWeight(values.weight, editingTheme.id)) {
            toast({
                variant: 'destructive',
                title: 'Жингийн нийлбэр 100%-иас хэтэрлээ',
                description: 'Бусад чиглэлийн жинг эхлээд бууруулна уу.',
            });
            return;
        }
