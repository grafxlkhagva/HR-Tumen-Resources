// src/app/dashboard/hr/business-plan/components/okr-tab.tsx
'use client';
...
import {
    Plus, Pencil, Target, ChevronRight, TrendingUp, FolderKanban, Link2
} from 'lucide-react';
...
export function OkrTab({
    ...
    // Projects — стратегийн холбоосоор шүүнэ
    const projectsQuery = useMemoFirebase(
        ({ firestore, companyPath }) => firestore ? tenantCollection(firestore, companyPath, 'projects') : null,
        []
    );
    const { data: allProjects } = useCollection<Project>(projectsQuery);

    // Objective ID-аар холбоотой projects map
    const projectsByObjectiveId = useMemo(() => {
        const map = new Map<string, Project[]>();
        allProjects.forEach(p => {
            const objId = p.strategyLink?.objectiveId;
            if (!objId) return;
            if (!map.has(objId)) map.set(objId, []);
            map.get(objId)!.push(p);
        });
        return map;
    }, [allProjects]);
                        ...
                        return (
                            <Card key={obj.id}>
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {obj.level && obj.level !== 'company' && (
                                                    <Badge className={cn('text-[10px]', OBJECTIVE_LEVEL_COLORS[obj.level])}>
                                                        {OBJECTIVE_LEVEL_LABELS[obj.level]}
                                                    </Badge>
                                                )}
                                ...
                                <CardContent className="pt-0">
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-3 flex-shrink-0">
                                                        ...
                                                        {kr.confidenceLevel !== undefined && (() => {
                                                            const conf = getConfidenceLabel(kr.confidenceLevel);
                                                            return (
                                                                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${conf.color}`}>
                                                                    {kr.confidenceLevel.toFixed(1)}
                                                                </span>
                                                            );
                                                        })()}
                                    ...
                                    {/* ── Linked Projects ── */}
                                    {(() => {
                                        const linkedProjects = projectsByObjectiveId.get(obj.id) ?? [];
                                        ...
                                        return (
                                            <div className="mt-3 pt-3 border-t">
                                                <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                                                    ...
                                                    Холбоотой төслүүд ({linkedProjects.length})
                                                </p>
                                                <div className="space-y-1.5">
                                                    {linkedProjects.map(p => {
                                                        const statusColor =
                                                            p.status === 'COMPLETED' ? 'text-emerald-600 bg-emerald-50' :
                                                            p.status === 'ACTIVE' ? 'text-blue-600 bg-blue-50' :
                                                            p.status === 'ON_HOLD' ? 'text-amber-600 bg-amber-50' :
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-2 min-w-0">
                                                                    ...
                                                                    <span className="text-sm font-medium text-slate-700 truncate group-hover:text-violet-700">
                                                                        {p.name}
                                                                    </span>
                                                                </div>
                                                                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ml-2 ${statusColor}`}>
                                                                    {p.status === 'COMPLETED' ? 'Дууссан' :
                                                                     p.status === 'ACTIVE' ? 'Идэвхтэй' :
                                                                     p.status === 'ON_HOLD' ? 'Зогссон' :
                                                                     p.status === 'DRAFT' ? 'Ноорог' : p.status}
                                                                </span>
                                                            </Link>
                                                        );
                                                    })}
