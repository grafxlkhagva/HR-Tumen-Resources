// src/app/dashboard/hr/business-plan/components/kpi-tab.tsx
'use client';

import React, { useState, useMemo } from 'react';
import { useFirebase, addDocumentNonBlocking, updateDocumentNonBlocking, useTenantWrite, useCollection, useMemoFirebase, tenantCollection } from '@/firebase';
...
export function KpiTab({
    ...
    // KPI history tatah
    const kpiHistoryQuery = useMemoFirebase(
        ({ firestore, companyPath }) =>
            firestore && activePlan
                ? query(
                    tenantCollection(firestore, companyPath, 'bp_kpi_history'),
    ...
    const { data: kpiHistory } = useCollection<{
        id: string; kpiId: string; value: number; previousValue: number;
        ragStatus: string; recordedAt: string;
    }>(kpiHistoryQuery);

    // KPI ID-аар history map
    const historyByKpiId = useMemo(() => {
        const map = new Map<string, { date: string; value: number }[]>();
        kpiHistory.forEach(h => {
            if (!map.has(h.kpiId)) map.set(h.kpiId, []);
            map.get(h.kpiId)!.push({
                date: h.recordedAt ? format(parseISO(h.recordedAt), 'MM/dd') : '',
                value: h.value,
            });
        });
        return map;
    ...
    const handleUpdate = (values: KpiFormValues) => {
        ...
        // currentValue өөрчлөгдсөн бол history-д бичнэ
        if (values.current !== editingKpi.current) {
            addDocumentNonBlocking(tCollection('bp_kpi_history'), {
                kpiId: editingKpi.id,
                planId: editingKpi.planId,
                value: values.current,
                previousValue: editingKpi.current,
                ragStatus: rag,
                                ...
                                <CardContent className="pt-5">
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <Button
                                            ...
                                            onClick={(e) => { e.stopPropagation(); setEditingKpi(kpi); setIsDialogOpen(true); }}
                                        >
                                    ...
                                    {/* Trend chart — expand хийхэд харагдана */}
                                    {expandedKpiId === kpi.id && (() => {
                                        const history = historyByKpiId.get(kpi.id) ?? [];
                                        // Одоогийн утгыг history-д нэмэх (хамгийн сүүлийн цэг)
                                        const chartData = [
                                            ...history,
                                            { date: 'Одоо', value: kpi.current },
                                        ];
                                        return (
                                            <div className="mt-3 pt-3 border-t">
                                                ...
                                                {chartData.length > 1 ? (
                                                    <ResponsiveContainer width="100%" height={80}>
                                                        <LineChart data={chartData}>
                                                            ...
                                                            <Tooltip
                                                                ...
                                                                formatter={(v: number) => [`${v} ${kpi.unit}`, 'Утга']}
                                                            />
                                                            <Line
                                                                type="monotone"
                                                                dataKey="value"
                                                                stroke={
                                                                    kpi.computedRag === 'green' ? '#10b981' :
                                                                    kpi.computedRag === 'amber' ? '#f59e0b' : '#ef4444'
                                                                }
                                                                strokeWidth={2}
                                                                dot={{ r: 3 }}
                                                            />
                                                    ...
                                                    <p className="text-xs text-muted-foreground text-center py-2">
                                                        Дата хуримтлагдаагүй байна. KPI шинэчлэх бүрт chart бүрдэнэ.
                                                    </p>
                                                )}
                                            </div>
                                        );
