...
import type { TmsRecurringTransport, TmsTransportCheckpoint } from '@/app/tms/types';
import type { OtCheckpointStageDef } from '../../one-time-transports/constants';
import { CheckpointPhotoGrid } from '../../one-time-transports/[id]/checkpoint-photo-grid';

const MAX_UPLOAD_SIZE = 5 * 1024 * 1024; // 5MB — dispatch upload-тай ижил хязгаар

interface RtCheckpointDialogProps {
  open: boolean;
...
/**
 * Давтамжит тээврийн нэг checkpoint шатыг баталгаажуулах/засах цонх —
 * checklist + зураг (Storage) + transit шатанд км/шатахуун + тэмдэглэл.
 */
export function RtCheckpointDialog({
  open,
  onOpenChange,
  transport,
  stage,
}: RtCheckpointDialogProps) {
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        for (const file of files) {
          ...
          const path = `tms/recurring-transports/${transport.id}/checkpoints/${stage.key}/${Date.now()}-${safeName}`;
  ...
  const handleSave = React.useCallback(async () => {
    ...
    try {
      ...
      // Transit-ийн км-ийг нийт зайд хуулна (OT-тэй ижил зан төлөв)
      if (stage.key === 'transit' && km !== '' && Number(km) > 0) {
        updates.totalDistanceKm = Number(km);
      }
      await updateDoc(doc(firestore, TMS_RECURRING_TRANSPORTS_COLLECTION, transport.id), updates);
    ...
    <AppDialog open={open} onOpenChange={onOpenChange}>
      <AppDialogContent size="lg" showClose>
        ...
        <AppDialogBody className="space-y-5">
          ...
          {stage.key === 'transit' && (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="rt-cp-km">🛣 Зарцуулсан км</Label>
                <Input
                  id="rt-cp-km"
                  type="number"
                  min={0}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="rt-cp-fuel">⛽ Шатахууны зардал (₮)</Label>
                <Input
                  id="rt-cp-fuel"
                  type="number"
                  min={0}
          ...
          {/* Тэмдэглэл */}
          <div className="space-y-1.5">
            <Label htmlFor="rt-cp-notes">Тэмдэглэл</Label>
            <Textarea
              id="rt-cp-notes"
              value={notes}
