...
import { OT_STATUS_MAP, OT_STATUS_ORDER } from '../constants';

/**
 * Статусын timeline — planned → in_progress → completed → invoiced → paid.
 * Цуцлагдсан бол timeline-ийн оронд rose banner харуулна.
 */
export function StatusTimeline({ transport }: { transport: TmsOneTimeTransport }) {
  ...
  const currentIdx = OT_STATUS_ORDER.indexOf(status);
  const invoicedIdx = OT_STATUS_ORDER.indexOf('invoiced');
          ...
          return (
            <React.Fragment key={step}>
              ...
              <div className="flex flex-col items-center shrink-0">
                ...
                <span className="text-xs mt-1 text-center">{OT_STATUS_MAP[step].label}</span>
              </div>
            </React.Fragment>
          );
        })}
