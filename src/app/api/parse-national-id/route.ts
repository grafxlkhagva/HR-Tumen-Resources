...
import { extractNationalIdFromImages } from '@/ai/id-card-ocr';
  ...
  try {
    ...
    if (frontImageDataUrl && frontImageDataUrl.startsWith('data:')) {
      ...
      return NextResponse.json(
        { error: 'Иргэний үнэмлэхний урд талын зураг оруулна уу (frontImageDataUrl эсвэл frontImageUrl).' },
        { status: 400 }
      );
    }
    ...
    const data = await extractNationalIdFromImages(
      frontDataUrl,
      frontType,
      backDataUrl,
      backType
    ...
    console.error('parse-national-id API error:', err);
    const message =
      err instanceof Error ? err.message : 'Иргэний үнэмлэх танихад алдаа гарлаа';
