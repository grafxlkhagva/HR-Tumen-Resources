...
import { notifyTaskAssigned } from '@/lib/notify-client';
...
export function EditTaskDialog({ open, onOpenChange, projectId, task, teamMemberIds }: EditTaskDialogProps) {
    ...
    const onSubmit = async (values: TaskFormValues) => {
        ...
        try {
            ...
            // Шинээр нэмэгдсэн assignee-нүүдэд notification + email явуулах
            const prevAssignees = task.assigneeIds ?? [];
            const newAssignees = values.assigneeIds.filter((id) => !prevAssignees.includes(id));
            const dueDateStr = format(values.dueDate, 'yyyy-MM-dd');
            for (const assigneeUid of newAssignees) {
                const emp = employees?.find((e) => e.id === assigneeUid);
                notifyTaskAssigned({
                    targetUid: assigneeUid,
                    employeeName: emp ? `${emp.lastName ?? ''} ${emp.firstName ?? ''}`.trim() : '',
                    taskTitle: values.title,
                    projectName: projectId, // projectId байна, project нэр fetch хийх шаардлагатай бол нэмж болно
                    projectId,
                    dueDate: dueDateStr,
